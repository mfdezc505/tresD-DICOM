...
check(await crossOff() && await page.locator('#btn-cross').isHidden(), 'en Render 3D la Cruz está desactivada (y oculta, v0.8.3)');
await page.click('[data-layout="vpAx"]'); await sleep(400);
check(await crossOff() && await page.locator('#btn-cross').isHidden(), 'en el axial a solas tampoco (v0.8.3: solo 2×2, 3D+cortes y en fila)');
await page.click('[data-layout="quad"]'); await sleep(400);
check(!(await crossOff()) && await page.locator('#btn-cross').isVisible(), 'en 2×2 vuelve a estar disponible');
